package fun.yizhierha.common.utils.file;
...
public class ExcelUtils {
    ...
    private static <T> List<ExcelClassField> getExcelClassFieldList(Class<T> clazz) {
        ...
        for (Method method : methods) {
            ExcelClassField cf = getExcelClassMethod(method);
            if (cf.getHasAnnotation() == 1) {
                hasExportAnnotation = true;
            }
            int sort = cf.getSort();
    ...
    private static ExcelClassField getExcelClassMethod(Method method) {
    ...
    private static <T> List<List<Object>> getSheetData(List<T> list) {
        Class<?> nowClass = list.get(0).getClass();
        // 获取表头字段
        List<ExcelClassField> excelClassFieldList = getExcelClassFieldList(nowClass);
        Class<?> superclass = nowClass.getSuperclass();
        if (superclass == BaseDTO.class){
            excelClassFieldList.addAll(getExcelClassFieldList(superclass));
        }
        ...
        // 获取表数据
        for (T t : list) {
            Map<String, Object> fieldDataMap = getFieldDataMap(t,false);
            if (superclass == BaseDTO.class){
                fieldDataMap.putAll(getFieldDataMap(t,true));
            }
    ...
    private static <T> Map<String, Object> getFieldDataMap(T t,boolean callSupper) {
        Map<String, Object> map = new HashMap<>();
        Field[] fields ;
        Method[] declaredMethods ;
        if (callSupper){
            fields = t.getClass().getSuperclass().getDeclaredFields();
            declaredMethods = t.getClass().getSuperclass().getDeclaredMethods();
        }else {
            fields = t.getClass().getDeclaredFields();
            declaredMethods = t.getClass().getDeclaredMethods();
        }
        ...
        try {
            ...
            for (Method method : declaredMethods) {
                if (method.isAnnotationPresent(ExcelExport.class)){
                    String fieldName = method.getName();
